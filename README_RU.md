...
**Форум-режим — Telegram как дашборд проектов**

Ваша Telegram-супергруппа становится структурированным рабочим пространством. Каждый проект — свой тред-топик. Задачи живут в отдельном топике Tasks. Уведомления — в топике Activity. Это не просто пульт управления — это организованный командный центр для всех ваших проектов.

**Как настроить:**
1. Создайте Telegram-супергруппу с включёнными Topics
2. Напишите `/connect` в группе — Studio подключится мгновенно
3. Studio автоматически создаёт структуру: топик **Tasks** для управления задачами, топик **Activity** для уведомлений, и отдельный топик для каждого вашего проекта

**Топики проектов** — каждая директория проекта получает свой тред. Пишите сообщение в топик проекта — Claude работает в его контексте. `/new` — новая сессия, `/history` — прошлые сессии, `/session N` — переключиться, `/files` — файлы, `/diff` — изменения, `/log` — git-история, `/stop` — остановить задачу. Полный опыт Studio внутри каждого Telegram-треда.

**Топик Tasks** — ваш Kanban прямо в Telegram. Напишите название задачи — она создана мгновенно. `/list` — увидеть все задачи, `/done ID` — отметить выполненной, `/start ID` — поставить в очередь на выполнение. Управляйте всем бэклогом без открытия браузера.

**Топик Activity** — каждое уведомление (задача выполнена, упала, сессионные события) постится сюда автоматически. Одно место, чтобы видеть всё, что произошло пока вас не было.

**Инлайн-создание** — в приватном чате с ботом кнопки **New Chat** и **New Task** создают элементы прямо на ходу. Никаких команд для запоминания — просто нажмите и пишите.
...
**Навыки из плагинов (🧩)** — Claude Code Studio автоматически обнаруживает навыки из ваших установленных плагинов Claude Code. Навыки из marketplace и кешированных плагинов появляются в сайдбаре, сгруппированные по имени плагина. Никакой конфигурации — установите плагин в Claude Code, и его навыки сразу доступны в Studio.
...
Не только Telegram. Откройте публичный URL вашей Studio (через Remote Access) в любом мобильном браузере — и вы получите опыт, как от нативного приложения.

**Нативный мобильный UI** — полный редизайн для маленьких экранов. Мобильный хедер с живым индикатором статуса (пульсирующий оранжевый пока Claude работает, зелёный в покое, красный при ошибке). Bottom sheet с настройками выезжает снизу — переключайте режимы, модели и типы агентов одним тапом. Вкладки сессий живут в полноэкранном оверлее с индивидуальными кнопками закрытия и счётчиком вкладок.

- **Оптимизировано для касаний** — 44px зоны нажатия, scroll-snap колонки Kanban, свайпабельные панели
- **iOS-safe** — без авто-зума при фокусе, safe area insets для устройств с вырезом, без rubber-banding
...
- **Доступность** — все модальные окна используют ARIA-совместимые focus traps с Escape-для-закрытия и восстановлением фокуса

Вы в поезде, туннель работает, открываете URL на телефоне. Полная Kanban-доска со snap-скроллом. Полный чат со стримингом. Настройки — свайп в сторону. Не «мобильная версия» — настоящий интерфейс, переработанный для касаний.

### 🔄 Самовосстанавливающиеся сессии

Если сессия Claude потеряна — перезапуск сервера, сетевая проблема, просроченная сессия — Studio автоматически воссоздаёт полный контекст разговора из истории SQLite и воспроизводит его в свежую сессию. Claude продолжает когерентно оттуда, где остановился. Никакого ручного вмешательства, никакого потерянного контекста. Работает для локальных и SSH сессий.

Когда Claude исчерпает свои авто-продолжения, кнопка **Restart Session** появляется прямо в чате. Один клик очищает сломанную сессию и позволяет продолжить работу.
...
| 🏛 Telegram Forum Mode | Интеграция с Topics супергруппы — каждый проект в своём треде, с топиками Tasks и Activity для структурированного управления |
...
| 🧩 Навыки из плагинов | Автоматическое обнаружение навыков из установленных плагинов Claude Code — без конфигурации |
...
| 📱 Мобильный UI | Нативный мобильный интерфейс — bottom sheet настроек, индикатор статуса, оверлей вкладок, свайп-жесты |
...
| 🛡 Самовосстанавливающиеся сессии | Полное воспроизведение сессии из истории SQLite при потере — автоматически восстанавливает контекст, продолжает бесшовно |
