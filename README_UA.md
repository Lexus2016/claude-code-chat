...
**Форум-режим — Telegram як дашборд проєктів**

Ваша Telegram-супергрупа стає структурованим робочим простором. Кожен проєкт — свій тред-топік. Задачі живуть в окремому топіку Tasks. Сповіщення — в топіку Activity. Це не просто пульт управління — це організований командний центр для всіх ваших проєктів.

**Як налаштувати:**
1. Створіть Telegram-супергрупу з увімкненими Topics
2. Напишіть `/connect` у групі — Studio підключиться миттєво
3. Studio автоматично створює структуру: топік **Tasks** для управління задачами, топік **Activity** для сповіщень, і окремий топік для кожного вашого проєкту

**Топіки проєктів** — кожна директорія проєкту отримує свій тред. Пишіть повідомлення у топік проєкту — Claude працює в його контексті. `/new` — нова сесія, `/history` — минулі сесії, `/session N` — перемкнутись, `/files` — переглянути файли, `/diff` — зміни, `/log` — git-історія, `/stop` — зупинити задачу. Повний досвід Studio всередині кожного Telegram-треду.

**Топік Tasks** — ваш Kanban прямо в Telegram. Напишіть назву задачі — вона створена миттєво. `/list` — побачити всі задачі, `/done ID` — позначити виконаною, `/start ID` — поставити в чергу на виконання. Керуйте всім беклогом без відкриття браузера.

**Топік Activity** — кожне сповіщення (задача виконана, впала, сесійні події) поститься сюди автоматично. Одне місце, щоб бачити все, що сталось поки вас не було.

**Інлайн-створення** — в приватному чаті з ботом кнопки **New Chat** та **New Task** створюють елементи прямо на ходу. Жодних команд для запамʼятовування — просто тапніть і пишіть.
...
**Навички з плагінів (🧩)** — Claude Code Studio автоматично виявляє навички з ваших встановлених плагінів Claude Code. Навички з marketplace та кешованих плагінів зʼявляються у сайдбарі, згруповані за назвою плагіна. Жодної конфігурації — встановіть плагін у Claude Code, і його навички одразу доступні в Studio.
...
**Вкладення працюють і віддалено** — Вставте скріншот, завантажте PDF або перетягніть будь-який файл у чат. Claude Code Studio автоматично завантажує їх на віддалений сервер через SFTP перед обробкою. Ваші віддалені сесії Claude мають повний доступ до зображень і файлів, як і локальні.
...
Не лише Telegram. Відкрийте публічний URL вашої Studio (через Remote Access) у будь-якому мобільному браузері — і отримаєте досвід, що нагадує нативний додаток.

**Нативний мобільний UI** — повний редизайн для малих екранів. Мобільний хедер з живим індикатором статусу (пульсуючий оранжевий коли Claude працює, зелений у спокої, червоний при помилці). Bottom sheet з налаштуваннями висувається знизу — перемикайте режими, моделі та типи агентів одним тапом. Вкладки сесій живуть у повноекранному оверлеї з індивідуальними кнопками закриття та лічильником вкладок.

- **Оптимізовано для дотику** — 44px зони натиску, scroll-snap колонки Kanban, свайпабельні панелі
- **iOS-safe** — без авто-зуму на фокус, safe area insets для пристроїв з виїмкою, без rubber-banding
...
- **Доступність** — усі модальні вікна використовують ARIA-сумісні focus traps з Escape-для-закриття та відновленням фокусу

Ви в поїзді, тунель працює, відкриваєте URL на телефоні. Повна Kanban-дошка з snap-скролом. Повний чат зі стрімінгом. Налаштування — свайп убік. Не «мобільна версія» — справжній інтерфейс, перероблений для дотику.

### 🔄 Самовідновлювальні сесії

Якщо сесія Claude втрачена — перезапуск сервера, мережева проблема, прострочена сесія — Studio автоматично відтворює повний контекст розмови з історії SQLite та повторює його в нову сесію. Claude продовжує когерентно звідти де зупинився. Жодного ручного втручання, жодного втраченого контексту. Працює для локальних та SSH сесій.

Коли Claude вичерпає свої авто-продовження, кнопка **Restart Session** зʼявляється прямо в чаті. Один клік очищує зламану сесію і дозволяє продовжити роботу.
...
| 🏛 Telegram Forum Mode | Інтеграція з Topics супергрупи — кожен проєкт у своєму треді, з топіками Tasks та Activity для структурованого управління |
...
| 🧩 Навички з плагінів | Автоматичне виявлення навичок з встановлених плагінів Claude Code — без конфігурації |
...
| 📱 Мобільний UI | Нативний мобільний інтерфейс — bottom sheet налаштувань, індикатор статусу, оверлей вкладок, свайп-жести |
...
| 🛡 Самовідновлювальні сесії | Повне відтворення сесії з історії SQLite при втраті — автоматично відновлює контекст, продовжує безшовно |
