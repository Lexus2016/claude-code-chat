...
<details>
<summary><b>Вирішення проблем: npm install з GitHub падає з помилкою</b></summary>

Деякі версії npm (особливо 10.x) падають при встановленні git-залежностей з помилкою:

```
npm error --prefer-online cannot be provided when using --prefer-offline
```

Це відомий баг npm, де при підготовці git-пакета передаються взаємовиключні прапорці.

**Обхідний шлях — встановити вручну:**
```bash
git clone https://github.com/Lexus2016/claude-code-studio.git
cd claude-code-studio
npm install
npm install -g .   # опціонально: зареєструвати CLI-команду `claude-code-studio` глобально
```
...
Натисніть **Перекласти** у модальному вікні мислення — і ланцюжок міркувань з'явиться мовою вашого інтерфейсу. Переклад виконується через Claude haiku і кешується — повторний виклик миттєвий. Кнопка **Копіювати** завжди копіює те, що відображається на екрані — оригінал або переклад.
...
- **Перенос між комп'ютерами** — експортуйте на десктопі, імпортуйте на ноутбуці чи сервері. Та сама розмова, інша машина.
- **Завантажити в будь-який AI-агент** — киньте експортований JSON у ChatGPT, Gemini або будь-який інший AI та скажіть «розберись, що ми обговорювали, і давай продовжимо». Формат зрозумілий людині та самодостатній.
- **Імпорт з будь-якого AI** — продуктивна сесія в іншому інструменті? Попросіть його зберегти діалог у форматі `{ "session": { "title": "..." }, "messages": [{ "role": "user"|"assistant", "type": "text", "content": "..." }] }` та імпортуйте результат у Studio. Ваші розмови не прив'язані до однієї платформи.

**Переривання завдання** — надішліть уточнення або нову інструкцію *поки Claude активно працює*, не зупиняючи поточне завдання. Компактна пілюля `⚡ Уточнити` з'являється в рядку введення під час генерації — натисніть, щоб переключитися між **Уточнити** (інжектувати в живий потік) та **Черга** (запланувати після поточного завдання). Доставка тепер *гарантована* через хук `PreToolUse`: Studio перехоплює кожен виклик інструменту і доставляє повідомлення до наступної дії Claude. Бейдж у реальному часі показує статус доставки. **Статус доставки тепер зберігається в SQLite** — перезавантажте сторінку, поверніться до сесії через кілька днів, і кожне переривання в історії коректно покаже, чи дійшло воно до Claude або завершилось без доставки. Стан бейджів звіряється з авторитетним лічильником сервера при завершенні завдання — коректно в будь-якому сценарії, включно з кількома вкладками та пропущеними подіями. Текст пілюлі оновлюється миттєво при зміні мови інтерфейсу — оновлення сторінки не потрібне. Пілюля **враховує тип сесії**: залишається прихованою для віддалених SSH-сесій (де функція не підтримується) і зникає одразу при перемиканні з локального проекту на SSH — жодних застарілих елементів у рядку введення.

**Вкладення до переривання** — прикріпіть файл, скріншот або SSH-конфіг до будь-якого уточнення просто під час завдання. Claude отримує зображення як візуальний контент (повні мультимодальні MCP-блоки), файли — як читабельні шляхи, SSH-конфіги — з уже заповненими обліковими даними. Не потрібно зупиняти завдання, щоб поділитися контекстом — все доставляється в тій же контрольній точці. Працює з веб-інтерфейсу та Telegram.

**Автоочікування rate limit** — коли API Claude відповідає rate limit або перевантаженням (429), Studio автоматично чекає скидання і повторює запит — без ручного оновлення, без втрати сесії. Живий зворотний відлік відображається прямо в чаті: *«Rate limited — повтор через 4 хв 30 с»*. До 3 автоматичних повторів, максимум 30 хвилин очікування.
...
Повторювані завдання тепер **перезбуджуються на місці** — той самий запис завдання скидається до наступного часу після кожного запуску, замість створення нового рядка в базі даних. ID завдань залишаються стабільними між повтореннями, база даних не росте нескінченно, а відновлення після збою коректно перезбуджує перервані повторювані завдання замість того, щоб позначати їх виконаними.

**60-секундний сторожовий таймер** сканує завдання, що застрягли в `in_progress` без живого воркера — якщо воркер завершився аварійно без очищення, сторож автоматично відновлює завдання (перезбуджує повторювані, скидає одноразові в `todo`). Форма створення завдання попередньо заповнює дату/час **прямо зараз** — не потрібно заповнювати порожній picker перед плануванням.
...
- **Черга переривань під час роботи:** надішліть повідомлення поки Claude зайнятий — воно потрапить прямо в чергу переривань, а не в тупиковий «зайнято». Claude підхопить його на наступній контрольній точці. Можна додавати файли.
- **Переадресація Ask User:** питання Claude посеред завдань з'являються як кнопки Telegram — торкніться для відповіді або надішліть файл/зображення як відповідь
...
Forum Mode тепер реалізовано у **виділеному самостійному модулі** (`telegram-bot-forum.js`). Кожен проектний топік працює у повністю ізольованому стані — перемикання між проектами в різних тредах ніколи не змішує контекст і сесії. Надійна робота з десятком активних Forum-топіків одночасно. Кнопка **Відкрити чат** тепер завжди використовує callback — бот коректно перемикає контекст сесії і показує превʼю чату, а не просто переходить до URL топіку.
...
### ⇗ Крос-агентне делегування

Передавайте завдання зовнішнім AI CLI — OpenAI Codex, Gemini CLI, opencode, Aider — прямо з інтерфейсу чату. Два режими:

| | Handoff | Sync |
|---|---|---|
| Процес | Передав і забув | Паралельна співпраця |
| Комунікація | Одностороння передача контексту | Двостороння через DIALOG.md |
| Моніторинг | Ручна перевірка | Авто-опитування кожні 15с + fs.watch |
| Найкраще для | Незалежних завдань | Завдань, що потребують діалогу |
...
| Модель | Контекст | Найкраще для |
|-------|---------|----------|
| **Haiku** | Стандартний | Швидкий — прості питання, швидкі перевірки |
| **Sonnet** | **1М токенів** | Збалансований (за замовчуванням) — більшість повсякденних завдань |
| **Opus** | **1М токенів** | Найпотужніший — складна архітектура, складні баги |

Sonnet та Opus працюють з **контекстним вікном у 1 мільйон токенів** — величезні кодові бази, довгі історії розмов і масивні набори файлів вміщуються в одну сесію без обмежень.
...
**ML/AI-інженери** — віддалені GPU-сервери через SSH. Черга навчальних завдань. Заплановані дата-пайплайни. Моніторинг з телефону через Telegram.
...
| **Telegram** | Керування ботом, push-сповіщення, переадресація ask_user (+ відповідь файлом), міст сесій, Forum Mode, inline stop, deep-link навігація, rich action buttons (EN/UA/RU), кнопка «Написати», вкладення файлів, черга переривань під час роботи |
...
| **Надійність** | Сесії з самовідновленням, захист від збоїв, атомарні записи, миттєва зупинка, автоочікування rate limit, безпека конкурентного доступу (блокування сесій + busy_timeout), авто-очищення зависаючих блокувань сесій |
